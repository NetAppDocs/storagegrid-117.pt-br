...
Para ser notificado de alertas assim que eles são acionados, você pode link:../installconfig/setting-up-email-notifications-for-alerts.html["configurar notificações por e-mail para alertas"] ou link:using-snmp-monitoring.html["Configurar traps SNMP"].
