...
Consulte as instruções de instalação para link:../rhel/index.html["Red Hat Enterprise Linux ou CentOS"] ou link:../ubuntu/index.html["Ubuntu ou Debian"] implantações.
...
* link:../rhel/configuring-host-network.html["Configurar a rede host (Red Hat Enterprise Linux ou CentOS)"]
