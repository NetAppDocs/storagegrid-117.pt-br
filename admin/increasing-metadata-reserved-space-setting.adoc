...
* Você está conectado ao Gerenciador de Grade usando um link:web-browser-requirements.html["navegador da web suportado"].
* Você tem o link:admin-group-permissions.html["Permissão de acesso root ou permissões de configuração da página de topologia de grade e outras configurações de grade"].
...
Para obter uma explicação detalhada de como a configuração Espaço Reservado para Metadados afeta o espaço permitido para armazenamento de metadados de objetos em um determinado Nó de Armazenamento, consulte link:managing-object-metadata-storage.html["Gerenciar o storage de metadados de objetos"].
