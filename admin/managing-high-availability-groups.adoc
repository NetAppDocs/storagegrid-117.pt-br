...
* Um grupo de HA que inclui apenas dispositivos SG100 ou SG1000 e nós de software baseados em VMware pode fornecer conexões altamente disponíveis para link:../admin/manage-s3-select-for-tenant-accounts.html["S3 inquilinos que usam S3 Select"]o . Os GRUPOS HA são recomendados ao usar S3 Select, mas não são necessários.
