...
. Opcionalmente, selecione quaisquer portas adicionais que você deseja abrir na rede cliente não confiável. Essas portas podem fornecer acesso ao Gerenciador de Grade, ao Gerenciador de Tenant ou a ambos.
+
...
NOTE: Essas portas adicionais estão abertas na rede do cliente, independentemente de estarem fechadas na guia Gerenciar acesso externo.
