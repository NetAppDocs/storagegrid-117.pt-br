...
Se encontrar um problema quando link:../admin/index.html["Administrar um sistema StorageGRID"], pode utilizar o processo descrito nesta figura para identificar e analisar o problema. Normalmente, você pode resolver problemas sozinho. No entanto, talvez seja necessário encaminhar alguns problemas para o suporte técnico.
...
* link:../monitor/manually-triggering-autosupport-message.html["Acione manualmente uma mensagem AutoSupport"]
...
* O ILM faz objetos replicados ou codificados para apagamento? O ILM garante a redundância do site? As regras do ILM usam os comportamentos de ingestão equilibrada, rigorosa ou dupla confirmação?
